    ...
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        for i in range(9):
            ...
            for j in range(9):
                ...
                #3x3 Check
                print(i, j, (2*(i//3)) + (i//3) + (j//3))
                if board[i][j] not in seen3x3[(2*(i//3)) + (i//3) + (j//3)] and board[i][j].isdigit():
                    seen3x3[(2*(i//3)) + (i//3) + (j//3)].add(board[i][j])
                elif board[i][j] in seen3x3[(2*(i//3)) + (i//3) + (j//3)] and board[i][j].isdigit():
                    return False
        return True